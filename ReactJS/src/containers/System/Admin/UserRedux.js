    ...
    componentDidUpdate(prevProps,prevState,snapshot){
        if(prevProps.genderRedux !== this.props.genderRedux){
            ...
            this.setState({
                ...
                gender:arrGenderes && arrGenderes.length>0 ? arrGenderes[0].keyMap : ''
            })
        }
                            ...
                            <div className='col-3'>
                                ...
                                <select className='form-control'  
                                    ...
                                    {genders && genders.length>0 && genders.map((item,index) => {
                                        return(
                                            <option key = {index} selected value={item.keyMap}>{language === LANGUAGES.VI ? item.valueVi : item.valueEn}</option>
                                        )
                                    })}
                                </select>
                            </div>
